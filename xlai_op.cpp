...
bool MoveAt(uint8_t p) 
 ...
 int main()
 {
	...
	while(1)
	{
		...
		fscanf(fin,"%d\n",&input_status);
		if(input_status == 1) // able to read input
		{
			first = 0;
			// read input
			for(i=0;i<BW*BW;i++)
			...
			else
			{
				...
				// write data
				FILE *fr2 = fopen("data.txt","a");
				fprintf(fr2,"%d %d ",p2,p);
				fclose(fr2);
				posnum += 2;
		...
		else if(input_status > 1)	// game end
		{
			printf("game  end\n");
			FILE *f6 = fopen("input.txt","w");
			FILE *f7 = fopen("output.txt","w");
			FILE *fr3 = fopen("data.txt","a");
			fprintf(f6,"1\n");
			for(i=0;i<BW*BW;i++)
			{
				fprintf(f6,"2\n");
